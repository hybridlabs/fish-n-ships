...
class DinghyWithDoubleChestItem(
    private val variant: DinghyEntity.Type,
    ...
    override fun use(level: Level, player: Player, hand: InteractionHand): InteractionResultHolder<ItemStack?> {
        ...
        if (hitresult.type == HitResult.Type.MISS) {
            ...
            if (hitresult.type == HitResult.Type.BLOCK) {
                val dinghy = getDinghy(level, hitresult)
                dinghy.variant = variant
                dinghy.yRot = player.yRot

                if (!level.noCollision(dinghy, dinghy.boundingBox)) {
                    return InteractionResultHolder.fail(itemstack)
                } else {
                    if (!level.isClientSide) {
                        level.addFreshEntity(dinghy)
    ...
    private fun getDinghy(level: Level, hitResult: HitResult): DinghyWithDoubleChestEntity {
        val dinghy = FSEntityTypes.DINGHY_WITH_DOUBLE_CHEST.get().create(level)
