...
class SupplyRaftItem(properties: Properties) : Item(properties) {
    ...
    override fun use(level: Level, player: Player, hand: InteractionHand): InteractionResultHolder<ItemStack?> {
        ...
        if (hitresult.type == HitResult.Type.MISS) {
            ...
            if (hitresult.type == HitResult.Type.BLOCK) {
                val raft: RaftEntity = this.getRaft(level, hitresult)
    ...
    private fun getRaft(level: Level, hitResult: HitResult): RaftEntity {
        val raft = FSEntityTypes.SUPPLY_RAFT.get().create(level)
