...
class FSConfigHandler(val file: File, val defaultConfig: FSConfig = FSConfig()) {
    val backupFile = file.parentFile.resolve("${file.name}-backup")

    var config: FSConfig = defaultConfig

    /**
     * Reads the config from the config file.
     * @return a data result of the config
     */
    fun read(): DataResult<Pair<FSConfig, JsonElement>> {
        val json = file.reader().use(JsonParser::parseReader)
        return FSConfig.CODEC.decode(JsonOps.INSTANCE, json)
    }
    ...
    fun save(): Boolean {
        val dataResult = FSConfig.CODEC.encodeStart(JsonOps.INSTANCE, config)
        val result = dataResult.result()
